...
public class UserService {
	...
	public Member updateUserInfo(String userId, UserpageRequest req) {
		...
		member.update(req.getUserId(), req.getUserName(), req.getUserNickname(), req.getUserTelNumber(),
				req.getUserEmail(), req.getUserJoinDate());
	...
	public boolean changePwd(String userId, String currentPwd, String newPwd, String newPwdConfirm) {
		...
		// (암호화된)디비에 있는 비번과 현재 입력한 비번이 같을 경우
		if (member != null && passwordEncoder.matches(currentPwd, member.getUserPwd())) {
